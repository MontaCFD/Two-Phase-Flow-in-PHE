...
import os
from PHE_functions import Fluid, pressure_coefficient_Estimator
...
pressure_drop_dir = os.path.join(plot_dir, 'pressure_drop')
if not os.path.exists(pressure_drop_dir):
    os.makedirs(pressure_drop_dir)
...
for beta in betas:
    subfolder = os.path.join(pressure_drop_dir, f'beta_{beta}')
    if not os.path.exists(subfolder):
        os.makedirs(subfolder)

    # Initialize pressure drop coefficients for this beta based on applicable methods
    estimator = pressure_coefficient_Estimator(beta, Re_range[0], amplitude, wave_length)  # Using Re_range[0] for initial estimation
    ...
    # Create a custom colormap for deviations under and over 50%
    ...
    # Create a binary array where 0 is for deviations under 50% and 1 is for deviations above 50%
    binary_deviations = np.where(mean_absolute_deviations > 50, 1, 0)
