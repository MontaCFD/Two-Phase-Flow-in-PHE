...
class PHE_geometry(object):
    ...
    def hydraulic_diameter(self) -> float:
        ...
        D_h = 2.0 * self.amplitude / phi * 1 / 1000.
        #print("D_h = ",D_h)
        return D_h
    ...
    def inlet_area(self) -> float:
        ...
        A_inlet = 2 * self.amplitude * self.width     #check again
        return A_inlet
...
class pressure_coefficient_Estimator:
    ...
    def friction_coeff_III(self):
        ...
        if 30 <= beta <= 60 and Re_h >= 1000:
            f = 4/phi * (2.917 - 0.1277 * beta + 2.016 * 10**(-3) * beta ** 2) * (5.474 - 19.02 * phi + 18.93 * phi**2 - 5.341 * phi**3) * (phi * Re_h)**(-(0.2 + 0.0577*sin(pi * beta/45 + 2.1)))
            return  f
    ...
    def friction_coeff_IV(self):
        ...
        z = 4.0  # = p/h --> Anpassen   ##TODO:verify
        #print('test')
        if 15 <= beta <= 75 and 2.0 <= z < 4.4 and 200 < Re_h < 10000:
            f = 4/phi * 5.9666 * ( pi / 180 * beta) ** (- 0.0767 * log(phi * Re_h) + 1.5858) * z ** (-(1.283 * 10**(-5) * phi * Re_h + 0.8886)) * (phi * Re_h) ** (-0.1718)
            #print( f)
            return f
    ...
    def friction_coeff_V(self):
        ...
        if beta == 0 and 2300 < Re_h < 56000:
            f = 1/phi * 0.552 * (phi * Re_h)**(-0.263)
            return f

        if beta == 30 and 260 < Re_h < 3000:
            f = 1/phi * (0.37 + 230/(phi * Re_h))
            return f

        if beta == 30 and 3000 <= Re_h < 50000:
            f = 1/phi * 3.59 * (phi * Re_h)**(-0.263)
            return f

        if beta == 45 and 150 < Re_h < 1800:
            f = 1/phi * (1.21 + 367/(phi * Re_h))
            return f

        if beta == 45 and 1800 <= Re_h < 30000:
            f = 1/phi * 5.84 * (phi * Re_h)**(-0.177)
            return f

        if beta == 60 and 90 < Re_h < 400:
            f = 1/phi * (5.03 + 755/(phi * Re_h))
            return f

        if beta == 60 and 400 <= Re_h < 16000:
            f = 1/phi * (26.8*(phi * Re_h)**(-0.209))
            return f

        if beta == 72 and 110 < Re_h < 500:
            f = 1/phi * (19.0 + 764/(phi * Re_h))
            return f

        if beta == 72 and 500 <= Re_h < 12000:
            f = 1/phi * 132 * (phi * Re_h)**(-0.296)
            return f

        if beta == 80 and 130 < Re_h < 3700:
            f = 1/phi * 140 * (phi * Re_h)**(-0.28)
            return f

        if beta == 90 and 200 < Re_h < 3000:
            f = 1/phi * (5.63 + 1280/(phi * Re_h))
            return f

        if beta == 90 and 3000 <= Re_h < 16000:
            f = 1/phi * 63.8*(phi * Re_h)**(-0.289)
            return f
    ...
    def friction_coeff_VIII(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Grabenstein et al.
        > https://doi.org/10.1016/j.ijmultiphaseflow.2017.01.012
        > Re < 2300, Re > 2300
            Args:
                chevron angle beta
                Reynolds number Re_h
    ...
    def friction_coeff_IX(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Gulenoglu et al.
        > https://doi.org/10.1016/j.ijthermalsci.2013.06.012
        > 300 ≤ Re_h ≤ 5000, beta = 30
            Args:
                chevron angle beta
                Reynolds number Re_h
            Returns:
        ...
        if beta == 30 and 300 <= Re_h <= 5000:
            ...
            xsi = 4/phi * (259.9 * (phi * Re_h)**(-0.9227) + 1.246)
    ...
    def friction_coeff_X(self):
        ...
        if 950 < Re_h < 1400 and beta == 65:
            ...
            xsi = 4 * phi**4 * (0.6796*phi*Re_h**(-0.0551)+0.2)

            return xsi

    def friction_coeff_XI(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Warnakulasuriya et al.
        > https://doi.org/10.1016/j.ijheatmasstransfer.2007.04.054
        > β = 60°, 250 < Re < 1100
            Args:
                chevron angle beta
                Reynolds number Re_h
        ...
        xsi = None
        if 250 < Re_h < 1100 and beta == 60:
            xsi = C * Re_h ** n

        return xsi

    def friction_coeff_XII(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Muley and Metwally (1999)
        > https://doi.org/10.1115/1.2826051
        > 30 < β < 60, 2 ≤ Re ≤ 300
            Args:
                chevron angle beta
                Reynolds number Re_h
        ...
        xsi = None
        if 2 <= Re_h <= 300 and 30 <= beta <= 60:
            xsi = 4/phi * ((30.20/(phi * Re_h))**5 + (6.28/((phi * Re_h)**0.5))**5)**0.2 * (beta/30)**0.83

        return xsi

    def friction_coeff_XIII(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Huang (2010)
        > https://core.ac.uk/download/pdf/39668662.pdf
        > β = 60°, 210 ≤ Re ≤ 1000
            Args:
                chevron angle beta
                Reynolds number Re_h
        ...
        xsi = None
        if 210 <= Re_h <= 1000 and beta == 60:
            xsi = C * Re_h ** n

        return xsi

    def friction_coeff_XIV(self):
        '''
        This function calculates the Darcy friction coefficient based on correlation of Passoni et al.
    ...
    def select_method(self, index):
        ...
        if index == 0:
            ...
            return self.friction_coeff_XI()
        elif index == 11:
            return self.friction_coeff_XII()
        elif index == 12:
            return self.friction_coeff_XIII()
        elif index == 13:
            return self.friction_coeff_XIV()
        else:
    ...
    def applicable_methods(self):
        ...
        if beta == 60 and 250 < Re_h < 1100:
            applicable_indices.append(10)
        if 2 <= Re_h <= 300 and 30 <= beta <= 60:
            applicable_indices.append(11)
        if 210 <= Re_h <= 1000 and beta == 60:
            applicable_indices.append(12)
        if 30 < Re_h < 1300 and beta == 63:
            applicable_indices.append(13)
    ...
    def get_correlation_name(self, index: int) -> str:
        ...
        correlations = {
            0: "Martin Holger",
            1: "Al zahrani, Islam und Saha",
            2: "Muley und Manglik",
            3: "Lee and Lee",
            4: "Focke, Zachariades und Olivier",
            5: "Zhu and Haglind",
            6: "Buscher",
            7: "Grabenstein",
            8: "Gulenoglu",
...
class Nusselt_estimator:
    ...
    def nusselt_II(self):
        ...
        phi = 1.14  #Annahme, dα keine genauen Angaben

        if beta == 60 and 500 < Re_h < 3000 and 0.72 < Pr < 7.5:
            Nu = 1/phi * 0.238 * (phi*Re_h) ** (0.6417) * Pr ** (1/3) * (mu/mu_w) ** (0.14)
            return Nu
    ...
    def nusselt_III(self):
        ...
        if 30 <= beta <= 60 and Re_h >= 1000:
            Nu = 1/phi * (0.2668 - 0.006967 * beta + 7.244 * 10 ** (-5) * beta ** 2) * (phi*Re_h) ** (0.728+0.0543 * sin(pi * beta/45 + 3.7)) * Pr**(1/3) * (my/my_w)**0.14
            return Nu
    ...
    def nusselt_IV(self):
        ...
        if beta in [30, 45, 60] and 2 < Pr < 50 and 200 < Re_h < 10000: # beta == 60 
            Nu = 1/phi * 0.1312 * ((phi*Re_h)**0.78) * Pr ** (1/3)
            return Nu
    ...
    def nusselt_VI(self):
        ...
        if 950 < Re_h < 1400 and beta == 65:
            Nu = 0.1452 * phi**2.079 * Re_h ** (0.7640) * Pr ** (1 / 3)
            return Nu
    ...
    def get_correlation_name(self, index):
        ...
        return correlations.get(index, "Invalid index! Please enter a value between 0 and 5.")
